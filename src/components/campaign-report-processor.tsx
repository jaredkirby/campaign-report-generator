...
export function CampaignReportProcessor() {
    ...
    const processReport = async () => {
        if (!file || primaryEmails.length === 0) {
            setError(
                "Please select a file and add at least one primary email recipient"
            );
            return;
        }
            ...
            if (result.data?.markdown_path) {
                ...
                try {
                    ...
                    if (!markdownResponse.ok) {
                        ...
                        throw new Error(
                            `Failed to fetch report: ${markdownResponse.statusText}`
                        );
                    }
                    ...
                    if (result.data?.campaign_count) {
                        console.log(
                            `Processed ${result.data.campaign_count} campaigns`
                        );
                    }
                    if (result.data?.changes_detected) {
                        console.log(
                            `Detected ${result.data.changes_detected} changes`
                        );
                    }
                } catch (err) {
                    if (err instanceof Error && err.name === "AbortError") {
                        throw new Error(
                            "Report fetch timed out - please try again"
                        );
                    }
                    throw err;
                }
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                ...
                                />
                                <Button
                                    onClick={() => addEmail("cc", newCCEmail)}
                                    size="icon"
                                >
                                    <Plus className="h-4 w-4" />
                                </Button>
